...
export default function ReminderCard({ reminder, onRemove, onBump, onToggle }: any) {
  ...
  return (
    <div className={`mb-3 border rounded-lg transition-all ${reminder.status === 'done' ? 'border-[var(--color-brand-sage)] border-opacity-30 opacity-60' : 'border-[var(--color-brand-graphite)] border-opacity-20 bg-[var(--color-background)]'}`}>
      ...
      >
        <div className="flex items-center space-x-3 flex-1">
          <button 
            ...
            className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${
              reminder.status === "done" 
                ? "bg-[var(--color-brand-sage)] border-[var(--color-brand-sage)] text-white" 
                : "border-[var(--color-brand-graphite)] border-opacity-40 hover:border-[var(--color-brand-amber)]"
            }`}
          >
          ...
          <div className="flex flex-col">
            <span className={`text-sm font-medium transition-all ${reminder.status === "done" ? "text-[var(--color-brand-graphite)] line-through opacity-70" : "text-[var(--color-foreground)]"}`}>
              {reminder.title}
            </span>
            <div className="flex items-center text-[10px] text-[var(--color-brand-graphite)] mt-1 space-x-2">
